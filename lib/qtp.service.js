...
var QTPParser = (function (_super) {
    ...
    QTPParser.prototype.processMsg = function () {
        ...
        for (; bufCount < this._oPool.length; ++bufCount) {
            peekBuf = this._oPool.peek(bufCount + 1);
            buflen += peekBuf[bufCount].byteLength;
            if (buflen >= this._curHeader.datalen + message_model_1.Header.len + this._curHeader.optslen) {
...
var QtpService = (function () {
    function QtpService() {
                ...
                for (var i = 0; i < msg.options.length; ++i) {
                    if (msg.options[i].id === message_model_1.OptionType.kSubscribeKey) {
                        ...
                        if (self._topicMap[msg.header.topic].context)
                            self._topicMap[msg.header.topic].callback.call(self._topicMap[msg.header.topic].context, key, msg.body, msg.options);
                        else
                            self._topicMap[msg.header.topic].callback(key, msg.body, msg.options);
                        break;
                    }
                }
                return;
        ...
        this._client.on("close", function (has_error) {
            ...
            // }, 10000);
            if (_this.onClose)
                _this.onClose(has_error[0]);
        });
        this._client.on("error", function (err) {
            if (_this.onError)
                _this.onError(err);
        });
        this._client.on("connect", function (e) {
            // if (_this._timer) {
            //     clearTimeout(_this._timer);
            //     _this._timer = null;
            // }
        ...
        this.addSlot({ service: 0, msgtype: 100, callback: () => { } });
    }
    ;
    QtpService.prototype.connect = function (port, host, isSSL) {
        ...
        this._client.connect(port, host, isSSL);
    };
    ...
    QtpService.prototype.sendWithOption = function (msgtype, options, body, service, version) {
        ...
        if (version)
            msg.header.version = version;
        msg.body = body;
        return this._client.sendMessage(msg);
    };
    QtpService.prototype.subscribeWithOption = function (topic, options, body, service, version, opposite) {
    ...
    QtpService.prototype.sendMessage = function (msg) {
        return this._client.sendMessage(msg);
    };
    QtpService.prototype.subscribe = function (topic, keys, opposite, service) {
        if (opposite === void 0) { opposite = false; }
    ...
    QtpService.prototype.sendToCMS = function (recActor, body) {
        ...
        return this.sendWithOption(12, [recOpt, reqOpt, sendOpt], body, message_model_1.ServiceType.kCMS);
    };
