...
var SSLSocket = (function (_super) {
    ...
    SSLSocket.prototype.connect = function (port, ip) {
        ...
        this._tlsSock.on("end", function () {
            logger.info("Receive an FIN packet from " + (_this._tlsSock.remoteAddress + ":" + _this._tlsSock.remotePort));
            _this.emit("end");
        });
        this._tlsSock.on("close", function (has_error) {
            console.warn(_this._ip + ":" + _this._port + " is closed " + (has_error ? "unexpected" : "successfully") + "!");
            _this.emit("close", has_error);
        });
    };
    /**
